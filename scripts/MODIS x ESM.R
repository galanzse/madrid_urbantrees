...
# ESM ~ MODIS
# dataframe
ESM_rs <- terra::resample(x=ESM, y=MODIS_DAY, method='sum')
...
a1 <- ggplot(aes(x=ESM/max(ESM)*100,y=Night), data=ESMxMOD) +
  geom_point() +
  geom_smooth(formula = y~poly(x, 2, raw=TRUE), method='lm', level=0.95) +
...
df_mod_date <- df_mod_date %>% filter(date < '2020-12-31')
